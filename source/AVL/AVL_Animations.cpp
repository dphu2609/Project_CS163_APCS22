...
void AVL::insertAnimation() {
    switch(mAnimationStep) {
        case 1: {
            ...
            createBackupTree();
            createTree();
            mSceneLayers[CodeBox]->getChildren()[0]->setMultipleContent(CodeContainer::codeHolder[Code::InsertAVL]);
            mAnimationStep++;
            break;
        }
        case 2: {
        ...
        case 17: {
            ...
            else if (mNodeList.size() != 1) getBalanceFactorPath(mOperationNode->parent, mRoot);
            else getBalanceFactorPath(mRoot, mRoot);
            resetNodeState();
            mAnimationStep = 18;
            break;
        }
...
void AVL::deleteAnimation() {
    switch (mAnimationStep) {
        case 1: {
            ...
            createTree();
            createBackupTree();
            mSceneLayers[CodeBox]->getChildren()[0]->setMultipleContent(CodeContainer::codeHolder[Code::DeleteAVL]);
            mAnimationStep++;
            break;
        }
...
void AVL::searchAnimation() {
    switch(mAnimationStep) {
        case 1: {
            ...
            createTree();
            createBackupTree();
            mSceneLayers[CodeBox]->getChildren()[0]->setMultipleContent(CodeContainer::codeHolder[Code::SearchAVL]);
            mAnimationStep++;
            break;
        }
        ...
        case 3: {
            if (mTravelPath[mTravelIndex].first->val < mInputQueue.front()) {
                mSceneLayers[CodeBox]->getChildren()[0]->activateLine({6, 7});
            }
            else if (mTravelPath[mTravelIndex].first->val > mInputQueue.front()) {
                mSceneLayers[CodeBox]->getChildren()[0]->activateLine({4, 5});
            }
            else {
...
void AVL::moveTreeAnimation(bool isAllowPause, float speed, int animationStepAfterFinish) {
    ...
    for (auto &node : mNodeList) {
        ...
        mSceneLayers[LeftEdges]->getChildren()[node->nodeIndex]->moveBy2Points(
            ...
            (node->left ? node->left->position : node->position) + sf::Vector2f(Size::NODE_RADIUS, Size::NODE_RADIUS), speed
        );
        mSceneLayers[RightEdges]->getChildren()[node->nodeIndex]->moveBy2Points(
            node->position + sf::Vector2f(Size::NODE_RADIUS, Size::NODE_RADIUS),
            (node->right ? node->right->position : node->position) + sf::Vector2f(Size::NODE_RADIUS, Size::NODE_RADIUS), speed
        );
    } 
