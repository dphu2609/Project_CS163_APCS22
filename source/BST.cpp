...
void BST::buildScene() {
    ...
    for (int i = 0; i < sampleData.size(); i++) {
        insert(mRoot, 1, 0, sf::Vector2f(Constant::WINDOW_WIDTH/2 - 40, 100), sampleData[i]);
    }
}
...
BST::Node* BST::insert(Node *&root, int height, bool isLeft, const sf::Vector2f &parentPos, int data) {
    if (root == nullptr) {
        ...
        if (height != 1) {
            if (isLeft) position += sf::Vector2f(-100, 100);
            else position += sf::Vector2f(100, 100);
        }
    ...
    if (data < root->val) {
        ...
        if (root->left == nullptr) {
            ...
            edgeLeft->setPositionByPoints(root->position + sf::Vector2f(40, 40), sf::Vector2f(root->position.x + 100, root->position.y - 100) + sf::Vector2f(40, 40));
            ...
            mSceneLayers[LeftEdges]->getChildren()[root->edgeLeftIndex]->setPositionByPoints(root->position + sf::Vector2f(40, 40), root->left->position + sf::Vector2f(-100, 0) + sf::Vector2f(40, 40));
            moveTree(root->left, sf::Vector2f(-100, 0));
        }
        ...
        if (root->right == nullptr) {
            ...
            edgeRight->setPositionByPoints(root->position + sf::Vector2f(40, 40), sf::Vector2f(root->position.x + 100, root->position.y + 100) + sf::Vector2f(40, 40));
            ...
            mSceneLayers[RightEdges]->getChildren()[root->edgeRightIndex]->setPositionByPoints(root->position + sf::Vector2f(40, 40), root->right->position + sf::Vector2f(100, 0) + sf::Vector2f(40, 40));
            moveTree(root->right, sf::Vector2f(100, 0));
        }
...
void BST::moveTree(Node* root, const sf::Vector2f &dis) {
    ...
    if (root->left) mSceneLayers[LeftEdges]->getChildren()[root->edgeLeftIndex]->setPositionByPoints(root->position + dis + sf::Vector2f(40, 40), root->left->position + dis + sf::Vector2f(40, 40));
    if (root->right)  mSceneLayers[RightEdges]->getChildren()[root->edgeRightIndex]->setPositionByPoints(root->position + dis + sf::Vector2f(40, 40), root->right->position + dis + sf::Vector2f(40, 40));
    root->position += dis;
